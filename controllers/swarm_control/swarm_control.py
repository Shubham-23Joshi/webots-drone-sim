...
class Mavic (Robot):
    ...
    K_VERTICAL_P = 3.5 #3.0        # P constant of the vertical PID.
    K_ROLL_P = 80.0           # P constant of the roll PID.
    K_PITCH_P = 50.0          # P constant of the pitch PID.

    MAX_YAW_DISTURBANCE = 2.0
    MAX_PITCH_DISTURBANCE = -3.2
    ...
    def __init__(self):
        ...
        # Emitter / Receiver for decentralized consensus
        # Requires corresponding devices added in the Mavic2Pro proto.
        try:
            self.emitter = self.getDevice("emitter")
            self.receiver = self.getDevice("receiver")
            self.receiver.enable(self.time_step)
        except Exception:
            # If devices are missing, disable comms gracefully.
            self.emitter = None
            self.receiver = None
        ...
        self.neighbors = {}  # id -> (x, y, timestamp)
        #self.center_estimate = np.array([0.0, 0.0])
        self.center_estimate = None
        self.last_consensus_time = 0.0
        self.consensus_interval = 0.5  # seconds
    ...
    def _broadcast_position(self, now):
        if not self.emitter:
            return
        if now - self.last_emission_time < self.emission_interval:
            return
        msg = f"{drone_id},{self.current_pose[0]:.4f},{self.current_pose[1]:.4f}"
        try:
    ...
    def _run_consensus(self, now):
        ...
        active = [(x, y) for (x, y, t) in self.neighbors.values() if now - t <= self.neighbor_timeout]
        # include self
        active.append((self.current_pose[0], self.current_pose[1]))
        pts = np.array(active)
        if pts.size == 0:
            return
        centroid = np.mean(pts, axis=0)
        self.center_estimate = centroid
    ...
    def move_to_target(self, waypoints, verbose_movement=False, verbose_target=False):
        ...
        """
            # If consensus is available, override provided waypoint to follow the formation center + local offset
        if self.center_estimate is not None:
            waypoints = [[float(self.center_estimate[0]) + target_x, float(self.center_estimate[1]) + target_y]]
    ...
    def run(self):
        ...
        while self.step(self.time_step) != -1:
            ...
            now = self.getTime()
            # comms / consensus ops
            self._broadcast_position(now)
            self._process_incoming(now)
            if now - self.last_consensus_time >= self.consensus_interval:
                self._run_consensus(now)
                # debug: show neighbor count and centroid
                nc = len([1 for (_,_,t) in self.neighbors.values() if now - t <= self.neighbor_timeout])
                print(f"Drone {drone_id} neighbors={nc}, centroid={self.center_estimate}")
